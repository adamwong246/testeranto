...
const problemScene = makeScene2D('problem', function* (view) {
  ...
  yield view.add(
    <>
      <Rect width={600} height={400} fill={'#333'} x={-300} y={0} radius={10} />
      <Rect width={600} height={400} fill={'#333'} x={300} y={0} radius={10} />
      <Rect width={200} height={200} fill={'#00ffaa'} x={0} y={-200} radius={100}>
        <Txt text={"Dev"} fontSize={40} fill={'#1a1a2e'} />
      </Rect>
    </>
  );
  ...
  for (let i = 0; i < 10; i++) {
    view.add(
      <Rect
        ...
        shadowOffsetY={5}
      >
        <Txt text={`Test Failed ${i + 1}`} fontSize={20} fill={'#fff'} />
      </Rect>
    );
...
const solutionScene = makeScene2D('solution', function* (view) {
  ...
  view.add(
    <>
      <Rect width={'100%'} height={'100%'} fill={'#1a1a2e'} />
      <Audio src="./src/revideo/audio/solution.mp3" volume={1} time={0} play={true} />
    </>
  );

  // LLM Context Window Visualization
  const contextWindow = createRef<Rect>();
  view.add(
    <Rect
      ref={contextWindow}
      width={800}
      ...
      opacity={0}
    >
      <Txt text={"LLM Context Window"} fontSize={30} fill={'#fff'} y={-230} />
      <Rect width={750} height={400} fill={'#1a1a2e'} />
    </Rect>
  );

  yield* all(
    contextWindow().opacity(1, 0.5),
    contextWindow().position.y(0, 0.5)
  );
  ...
  for (let i = 0; i < 20; i++) {
    view.add(
      <Rect
        ...
        opacity={0}
      >
        <Txt text={`file${i}.ts`} fontSize={12} fill={'#fff'} />
      </Rect>
    ).opacity(1, 0.2).to(0.3, 2);
  }
  ...
  for (let i = 0; i < 3; i++) {
    view.add(
      <Rect
        ...
        opacity={0}
      >
        <Txt text={`relevant${i}.ts`} fontSize={16} fill={'#1a1a2e'} />
      </Rect>
    ).opacity(1, 0.3);
  }
  ...
  yield* chain(
    waitFor(durations.solution * 0.2),
    all(
      text.opacity(1, 0.5),
...
const contextScene = makeScene2D('context', function* (view) {
  ...
  view.add(
    <>
      <Rect width={'100%'} height={'100%'} fill={'#1a1a2e'} />
      <Audio src="./src/revideo/audio/context.mp3" volume={1} time={0} play={true} />
    </>
  );

  // Simple comparison visualization
  const messySide = createRef<Rect>();
  const cleanSide = createRef<Rect>();
  ...
  // Animate comparison
  yield* all(
    messySide().opacity(0.7, 0.5),
    cleanSide().opacity(1, 0.5)
  );
...
const runtimeScene = makeScene2D('runtimes', function* (view) {
  ...
  const runtimesAudio = view.add(
    <Audio
      src="./src/revideo/audio/runtimes.mp3"
      volume={1}
      playbackRate={1}
      time={0}
      play={true}
...
const ctaScene = makeScene2D('cta', function* (view) {
  ...
  // Animate in
  const ctaAudio = view.add(
    <Audio
      src="./src/revideo/audio/cta.mp3"
      volume={1}
      playbackRate={1}
      time={0}
      play={true}
...
const typeSafetyScene = makeScene2D('type-safety', function* (view) {
  ...
  const typeSafetyAudio = view.add(
    <Audio
      src="./src/revideo/audio/type-safety.mp3"
      volume={1}
      playbackRate={1}
      time={0}
      play={true}
