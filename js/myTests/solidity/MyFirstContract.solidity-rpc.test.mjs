...
var SolidityRpcTesteranto = (testImplementations, testSpecifications, testInput, contractName) => Testeranto(
  ...
  { ports: 1 },
  {
    beforeAll: () => __async(void 0, null, function* () {
      return (yield solCompile(contractName)).contracts.find((c) => c.contractName === contractName);
    }),
    ...
    beforeEach: (contract, i, tr) => {
      return new Promise((res) => {
        ...
        server.listen(port, (err) => __async(void 0, null, function* () {
          ...
          const contractNearSide = yield new web3NearSide.eth.Contract(contract.abi).deploy({ data: contract.bytecode.bytes }).send({ from: accounts[0], gas: 7e6 });
    ...
    afterEach: (_0) => __async(void 0, [_0], function* ({ server }) {
      return yield server.close();
    }),
    andWhen: (_0, _1) => __async(void 0, [_0, _1], function* ({ contractFarSide, accounts }, callback) {
      return callback()({ contractFarSide, accounts });
    })
  }
);
...
var MyFirstContractPlusRpcTesteranto = SolidityRpcTesteranto(
  {
    ...
    Whens: {
      Increment: (asTestUser) => (_0) => __async(void 0, [_0], function* ({ contractFarSide, accounts }) {
        return yield contractFarSide.inc({ gasLimit: 15e4 });
      }),
      Decrement: (asTestUser) => (_0) => __async(void 0, [_0], function* ({ contractFarSide, accounts }) {
        return yield contractFarSide.dec({ gasLimit: 15e4 });
      })
    },
    Thens: {
      Get: ({ asTestUser, expectation }) => (_0) => __async(void 0, [_0], function* ({ contractFarSide, accounts }) {
        return assert.equal(expectation, parseInt(yield contractFarSide.get({ gasLimit: 15e4 })));
      })
    },
    Checks: {
