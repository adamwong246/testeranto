...
var PuppeteerHttpTesteranto = (testImplementations, testSpecifications, testInput) => Testeranto(
  ...
  { ports: 1 },
  {
    ...
    andWhen: async function(store, actioner, testResource) {
      const [path2, body] = actioner(store);
      const y = await fetch(
        `http://localhost:${testResource.ports[0]}/${path2}`,
        {
          method: "POST",
          body
        }
    ...
    butThen: async function(store, callback, testResource) {
      const [path2, expectation] = callback(store);
      const bodytext = await (await fetch(`http://localhost:${testResource.ports[0]}/${path2}`)).text();
