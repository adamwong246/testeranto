...
var Http2xTesteranto = (testImplementations, testSpecifications, testInput) => Testeranto(
  ...
  { ports: 2 },
  {
    ...
    andWhen: async function(store, actioner, testResource) {
      const [path2, body, portSlot] = actioner(store);
      const y = await fetch(
        `http://localhost:${testResource.ports[portSlot]}/${path2}`,
        {
          method: "POST",
          body
        }
    ...
    butThen: async function(store, callback, testResource) {
      const [path2, expectation, portSlot] = callback({});
      const bodytext = await (await fetch(`http://localhost:${testResource.ports[portSlot]}/${path2}`)).text();
