                ...
                if [ "${runtime}" = "node" ]; then
                  ...
                  if [ -d "$BUNDLES_DIR" ]; then
                    find "$BUNDLES_DIR" -name "*.test.mjs" -type f | while read testfile; do
                      ...
                      cd /workspace
                      # Run test with default port 3456 and empty test resources
                      node "$testfile" 3456 '{}' || echo "Test $testfile exited with non-zero code"
                    done
                  else
                ...
                if [ "${runtime}" = "python" ]; then
                  ...
                  if [ -d "$BUNDLES_DIR" ]; then
                    find "$BUNDLES_DIR" -name "*.test.py" -type f | while read testfile; do
                      ...
                      cd /workspace
                      # Run test with default port 3456 and empty test resources
                      python3 "$testfile" 3456 '{}' || echo "Test $testfile exited with non-zero code"
                    done
                  else
                    echo "BUNDLES_DIR $BUNDLES_DIR does not exist"
                  fi
