...
var aiderPoolService_default = {
  ...
  ports: [
    "8765:8765",
    // WebSocket port for pool management
    "9000-9100:9000-9100"
    // Terminal ports for aider instances
  ],
  command: [
    "sh",
    "-c",
    `
    echo "=== Installing dependencies for aider pool ==="
    # Update pip first
    pip install --quiet --upgrade pip || echo "pip upgrade may have issues"
    
    # Install websockets (required for WebSocket server)
    pip install --quiet websockets || echo "websockets installation may have issues"
    
    # Install aider-chat (the package name on PyPI)
    echo "Installing aider-chat..."
    pip install --quiet aider-chat || {
        echo "Failed to install aider-chat via pip, trying alternative..."
        # Try installing from GitHub as fallback
        pip install --quiet "git+https://github.com/paul-gauthier/aider.git" || echo "All installation attempts failed"
    }
...
try:
    import aider
    print('\u2713 aider module imported successfully')
    print(f'aider location: {aider.__file__}')
except ImportError as e:
    print(f'\u2717 aider import failed: {e}')
    sys.exit(1)
...
"
    ...
    echo "Python script path: src/server/aider/aider_service.py"
...
async function generateServices(config, runtimes, webSocketPort, log, error) {
  ...
  services["aider-pool"] = aiderPoolService_default;
