...
var aiderPoolService_default = {
  ...
  command: [
    ...
    # Check if .aider.conf.yml exists in the container
    echo "=== Checking for .aider.conf.yml ==="
    if [ -f "/workspace/.aider.conf.yml" ]; then
      echo "\u2713 .aider.conf.yml file found in container"
      echo "File contents (first 5 lines):"
      head -5 /workspace/.aider.conf.yml || echo "Could not read file"
    else
      echo "\u26A0 .aider.conf.yml file not found in container"
    fi
    ...
    if [ -n "$OPENAI_API_KEY" ] || [ -n "$ANTHROPIC_API_KEY" ] || [ -n "$DEEPSEEK_API_KEY" ]; then
