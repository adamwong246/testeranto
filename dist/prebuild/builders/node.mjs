...
${file}:${line}:${column}: ERROR: ${pluginText}${e.text}`;
      ...
      if (path6.basename(nodeModulesDirectory) === "node_modules") {
        for (const unixKey in knownUnixlikePackages) {
          try {
            ...
            if (fs4.existsSync(path6.join(nodeModulesDirectory, pkg)))
              return pkg;
          } catch {
          }
        }
    ...
    function downloadedBinPath(pkg, subpath) {
      const esbuildLibDir = path6.dirname(__require.resolve("esbuild"));
      ...
      if (/\.zip\//.test(binPath)) {
        ...
        if (pnpapi) {
          ...
          const binTargetPath = path6.join(
            ...
            ".cache",
            "esbuild",
            `pnpapi-${pkg.replace("/", "-")}-${"0.20.2"}-${path6.basename(subpath)}`
          );
          if (!fs4.existsSync(binTargetPath)) {
            fs4.mkdirSync(path6.dirname(binTargetPath), { recursive: true });
