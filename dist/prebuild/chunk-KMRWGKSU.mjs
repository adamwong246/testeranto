...
async function pollForFile(path16, timeout = 2e3) {
  ...
  if (fs4.existsSync(stdoutPath)) {
    try {
      ...
      const testResults = {
        ...
        features: [],
        givens: [],
        fullPath: path2.resolve(process.cwd(), src)
      };
  ...
  async launchGolang(src, dest) {
    ...
    const golangPromise = (async () => {
      try {
        ...
        let currentDir = path3.dirname(src);
        let goModDir = null;
        while (currentDir !== path3.parse(currentDir).root) {
          if (fs5.existsSync(path3.join(currentDir, "go.mod"))) {
            goModDir = currentDir;
            break;
          }
        ...
        if (!goModDir) {
          ...
          goModDir = path3.dirname(src);
          console.error(`Falling back to: ${goModDir}`);
        }
        const relativeTestPath = path3.relative(goModDir, src);
        const child = spawn(
          "go",
          ["test", "-v", "-json", "./" + path3.dirname(relativeTestPath)],
          {
    ...
    const nodePromise = (async () => {
      try {
        ...
        if (fs6.existsSync(builtfile)) {
          ...
          const dir = path4.dirname(builtfile);
        ...
        if (!fs6.existsSync(builtfile)) {
          ...
          if (fs6.existsSync(dir)) {
            ...
            if (mjsFiles.length > 0) {
              const alternativePath = path4.join(dir, mjsFiles[0]);
...
// src/server/runtimes/golang/setupDockerfileForBuildGolang.ts
...
// src/server/runtimes/node/setupDockerfileForBuildNode.ts
...
// src/server/runtimes/python/setupDockerfileForBuildPython.ts
...
// src/server/runtimes/web/setupDockerfileForBuildWeb.ts
...
var WEB_TEST_FILES_PATH = {
  ...
  BASE_DIR: path5.join("testeranto", "bundles", "allTests", "web")
};
var CONTENT_TYPES = {
  PLAIN: "text/plain",
  HTML: "text/html",
...
var Server_Base = class {
  ...
  async writeFileSync(...x) {
    ...
    return new Promise(async (res) => {
      fs8.mkdirSync(path8.dirname(filepath), {
        recursive: true
      });
      if (!files[testName]) {
        files[testName] = /* @__PURE__ */ new Set();
...
var Server_TCP_Http = class extends Server_TCP_Core {
  ...
  serveStaticFile(req, res) {
    ...
    const normalizedPath = path9.normalize(relativePath);
    ...
    const filePath = path9.join(projectRoot, normalizedPath);
    if (!filePath.startsWith(path9.resolve(projectRoot))) {
      res.writeHead(403);
      res.end("Forbidden");
      return;
    }
      ...
      if (stats.isDirectory()) {
        fs9.readdir(filePath, (readErr, files2) => {
          ...
          const items = files2.map((file) => {
            try {
              const stat = fs9.statSync(path9.join(filePath, file));
              const isDir = stat.isDirectory();
              const slash = isDir ? "/" : "";
              return `<li><a href="${path9.join(
                urlPath,
                file
...
var Server_DockerCompose = class extends Server_TCP_Commands {
  ...
  async initializeAndStart() {
    const { setupDockerCompose } = await import("./dockerComposeGenerator-YRPJDZ7A.mjs");
...
var makeHtmlTestFiles = (testsName2) => {
  ...
  for (const sourceFilePath of webTests) {
    ...
    const htmlFilePath = path13.normalize(
    ...
    fs17.mkdirSync(path13.dirname(htmlFilePath), { recursive: true });
    fs17.writeFileSync(
      htmlFilePath,
...
var makeHtmlReportFile = (testsName2, config2) => {
  ...
  for (const sourceFilePath of tests) {
    ...
    for (const runtime of applicableRuntimes) {
      ...
      fs18.mkdirSync(path14.dirname(htmlFilePath), { recursive: true });
      const htmlDir = path14.dirname(htmlFilePath);
      const reportJsPath = path14.join(
        process.cwd(),
        "dist",
        "prebuild",
        "Report.js"
      );
      const relativeReportJsPath = path14.relative(htmlDir, reportJsPath);
      const relativeReportJsUrl = relativeReportJsPath.split(path14.sep).join("/");
      const reportCssPath = path14.join(
        process.cwd(),
        "dist",
        "prebuild",
        "Report.css"
      );
      const relativeReportCssPath = path14.relative(htmlDir, reportCssPath);
      const relativeReportCssUrl = relativeReportCssPath.split(path14.sep).join("/");
...
var testsName = path15.basename(configFilepath).split(".").slice(0, -1).join(".");
