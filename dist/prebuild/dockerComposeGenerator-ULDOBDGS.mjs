...
var serviceConfigCommand_default = (runtime, testPath, betterTestPath, webSocketPort = "3000") => {
  let script;
  switch (runtime) {
    case "node":
...
CMD ["npx", "tsx", "${runtime}.mjs", "allTests.ts", "dev"]`;
  ...
  for (const runtime of runtimes) {
    ...
    for (const [testPath, testConfig] of Object.entries(tests)) {
      ...
      serviceConfig.environment.WS_PORT = webSocketPort?.toString() || "3000";
      serviceConfig.command = serviceConfigCommand_default(
        runtime,
        testPath,
        betterTestPath,
        webSocketPort?.toString() || "3000"
      );
