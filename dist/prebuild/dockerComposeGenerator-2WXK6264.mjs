...
function generateWebTestCommand(runtime, testPath, betterTestPath, dockerManPort, webSocketPort) {
  ...
  for (const runtime of runtimes) {
    ...
    for (const [testPath, testConfig] of Object.entries(tests)) {
      ...
      if (runtime === "web") {
        ...
        serviceConfig.depends_on["web-build"] = {
        ...
        if (dockerManPort) {
          serviceConfig.environment["DOCKERMAN_PORT"] = dockerManPort.toString();
        }
        if (webSocketPort) {
          serviceConfig.environment["WEBSOCKET_PORT"] = webSocketPort.toString();
        }
        if (!serviceConfig.environment["DOCKERMAN_HOST"]) {
          serviceConfig.environment["DOCKERMAN_HOST"] = "host.docker.internal";
        }
