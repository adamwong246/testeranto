...
function parseAiderConfig() {
  const pathsToCheck = [
    path3.join(process.cwd(), ".aider.conf.yml"),
    path3.join(process.cwd(), ".aider.conf.yaml"),
    path3.join(process.env.HOME || process.env.USERPROFILE || "", ".aider.conf.yml"),
    path3.join(process.env.HOME || process.env.USERPROFILE || "", ".aider.conf.yaml")
  ];
  for (const configPath of pathsToCheck) {
    try {
      if (fs3.existsSync(configPath)) {
        ...
        console.log(`Loaded aider config from: ${configPath}`);
        return config;
      }
    } catch (error) {
      console.warn(`Failed to parse aider config from ${configPath}:`, error);
    }
  }
  console.log("No aider config file found");
  return null;
}
function extractApiKeys(config) {
  ...
  if (typeof config["api-key"] === "string") {
    ...
    config["api-key"].forEach((keyEntry) => {
...
function loadAiderApiKeys() {
  try {
    ...
    if (config) {
      ...
      console.log("Loaded API keys from .aider.conf.yml:", Object.keys(envVars));
      return envVars;
    }
  } catch (error) {
...
var aiderPoolService_default = {
  ...
  command: [
    ...
    "-c",
    `
    echo "=== Starting aider pool service ==="
    echo "Current directory: $(pwd)"
    echo "Workspace mounted at: /workspace"
    
    # Configure git for aider
    cd /workspace
    git config --global user.name "Testeranto Bot"
    git config --global user.email "bot@testeranto.local"
    ...
    if [ -z "$OPENAI_API_KEY" ] && [ -z "$ANTHROPIC_API_KEY" ] && [ -z "$DEEPSEEK_API_KEY" ]; then
      ...
      echo "Checking for .aider.conf.yml file..."
      if [ -f "/workspace/.aider.conf.yml" ]; then
        echo "Found .aider.conf.yml file"
        # The API keys should have been loaded from the config file
        # If they're still not set, aider will prompt for them
      else
        echo "No .aider.conf.yml file found"
        echo "Aider will prompt for API keys when needed"
      fi
    else
      echo "API keys configured from environment variables"
    fi
    
    # Test that aider works
    echo "=== Testing aider installation ==="
    if command -v aider &> /dev/null; then
      echo "\u2713 Aider is installed"
      aider --version || echo "Aider version check failed"
    else
      echo "\u2717 Aider command not found"
      echo "Checking Python packages..."
      pip list | grep aider || echo "aider not found in pip list"
      exit 1
    fi
    
    echo "=== Starting aider pool manager ==="
    # Run the aider pool manager script
    exec python3 -u src/server/aider/aider_service.py
  `
  ]
};
...
async function generateServices(config, runtimes, webSocketPort, log, error) {
  ...
  services["aider-pool"] = {
    ...
    env_file: [".env", ".aider.conf.yml"]
    // Load .env and .aider.conf.yml files
  };
  ...
  const command = `
    ...
    case "${runtime}" in
      "node"|"web")
        ...
        if [ -n "$(echo '${JSON.stringify(config.checks || {})}' | grep -v '^[{}]*$')" ]; then
