...
menuBox.on("select", (item, index) => {
  ...
  logBox.log(`{cyan-fg}${(/* @__PURE__ */ new Date()).toLocaleTimeString()}{/cyan-fg} ${selected.action}`);
  ...
  if (selected.name === "exit") {
    ...
    setTimeout(() => {
      logBox.log(`{green-fg}${(/* @__PURE__ */ new Date()).toLocaleTimeString()}{/green-fg} ${selected.name} completed.`);
