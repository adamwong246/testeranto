...
function createBuildService(runtime, dockerfileDir, testsName) {
  ...
  if (strategy === "combined-build-test-process-pools") {
    serviceConfig.command = [
      "sh",
      "-c",
      `echo "Building ${runtime} tests..." &&        npx tsx dist/prebuild/server/builders/${runtime}.mjs allTests.ts dev &&        echo "Running ${runtime} tests using process pool..." &&        # Run the built test files with appropriate arguments
       if [ "${runtime}" = "node" ]; then          echo "Looking for built test files in /workspace/testeranto/bundles/allTests/node..." &&          # Use the same port as the build container (3456) and empty test resources for now
         # In a real implementation, we would need to get test resources from somewhere
         PORT=3456 &&          TEST_RESOURCES='{}' &&          find /workspace/testeranto/bundles/allTests/node -name "*.mjs" -type f | while read testfile; do            echo "Running test: $testfile with port $PORT" &&            cd /workspace &&            node "$testfile" "$PORT" "$TEST_RESOURCES" || echo "Test $testfile exited with code $?";          done;        elif [ "${runtime}" = "python" ]; then          echo "Looking for Python test files..." &&          find /workspace/testeranto/bundles/allTests/python -name "*.py" -type f | while read testfile; do            echo "Running test: $testfile" &&            cd /workspace &&            python "$testfile" || echo "Test $testfile exited with code $?";          done;        else          echo "No test runner configured for ${runtime}";        fi &&        # Keep container alive for health checks
       echo "Test execution complete. Keeping container alive..." &&        sleep infinity`
    ];
...
async function generateServices(config, runtimes, webSocketPort, log, error) {
  ...
  for (const runtime of runtimes) {
    ...
    if (!hasTests)
      continue;
    const buildService = await import("./buildService-VUYSFGIV.mjs");
