...
var web_default = (config, entryPoints, testName, bundlesDir) => {
  ...
  for (const entryPoint of entryPoints) {
    ...
    const absoluteEntryPoint = path.isAbsolute(entryPoint) ? entryPoint : path.resolve(process.cwd(), entryPoint);
    if (!fs.existsSync(absoluteEntryPoint)) {
      console.error(`[web esbuild] ERROR: Entry point not found: ${absoluteEntryPoint}`);
      console.error(`[web esbuild] Current working directory: ${process.cwd()}`);
      const possiblePaths = [
        absoluteEntryPoint,
        path.resolve(process.cwd(), "src", entryPoint),
        path.resolve(process.cwd(), "..", entryPoint)
      ];
      for (const possiblePath of possiblePaths) {
        if (fs.existsSync(possiblePath)) {
          console.error(`[web esbuild] Found at alternative path: ${possiblePath}`);
          entryPointsObj[outputKey] = possiblePath;
          break;
        }
      }
      if (!entryPointsObj[outputKey]) {
