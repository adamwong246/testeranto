  ...
  "src/server/docker/buildService.ts"() {
    buildService_default = (runtime) => {
      return {
        ...
        extra_hosts: runtime === "node" ? ["host.docker.internal:host-gateway"] : void 0,
        command: [
          "sh",
          "-c",
          `echo 'Starting ${runtime} build in watch mode...'; 
                ...
                echo "Starting WebSocket server for tests..."
                cd /workspace
                
                # Check if Server_TCP_Commands exists
                if [ -f "dist/prebuild/server/Server_TCP_Commands.mjs" ]; then
                  echo "Starting WebSocket server on port 3456..."
                  # Start in background
                  npx tsx dist/prebuild/server/Server_TCP_Commands.mjs 3456 > /tmp/websocket.log 2>&1 &
                  WS_PID=$!
                  ...
                  if ps -p $WS_PID > /dev/null; then
                    ...
                    cat /tmp/websocket.log || echo "No WebSocket log"
                  fi
                else
                  echo "Warning: Server_TCP_Commands.mjs not found at dist/prebuild/server/Server_TCP_Commands.mjs"
                  echo "Current directory: $(pwd)"
                  echo "Listing dist/prebuild/server/:"
                  ls -la dist/prebuild/server/ 2>/dev/null || echo "Cannot list directory"
                fi
                ...
                if [ "${runtime}" = "node" ]; then
                  ...
                  if [ -d "$BUNDLES_DIR" ]; then
                    find "$BUNDLES_DIR" -name "*.test.mjs" -type f | while read testfile; do
                      ...
                      # Include WebSocket connection info for tests running in container
                      TEST_RESOURCES='{
                        "wsHost": "localhost",
                        "wsPort": 3456,
                        "ports": [3456],
