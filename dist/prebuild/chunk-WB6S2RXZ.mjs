...
async function pollForFile(path17, timeout = 2e3) {
...
var Server_DockerCompose = class extends Server_TCP_Commands {
  ...
  async initializeAndStart() {
    const { setupDockerCompose } = await import("./dockerComposeGenerator-FUNVS4BG.mjs");
...
var makeHtmlReportFile = (testsName2, config2) => {
  ...
  for (const sourceFilePath of tests) {
    ...
    for (const runtime of applicableRuntimes) {
      ...
      fs18.mkdirSync(path15.dirname(htmlFilePath), { recursive: true });
      const htmlDir = path15.dirname(htmlFilePath);
      const reportJsPath = path15.join(
        process.cwd(),
        "dist",
        "prebuild",
        "Report.js"
      );
      const relativeReportJsPath = path15.relative(htmlDir, reportJsPath);
      const relativeReportJsUrl = relativeReportJsPath.split(path15.sep).join("/");
      const reportCssPath = path15.join(
        process.cwd(),
        "dist",
        "prebuild",
        "Report.css"
      );
      const relativeReportCssPath = path15.relative(htmlDir, reportCssPath);
      const relativeReportCssUrl = relativeReportCssPath.split(path15.sep).join("/");
...
var testsName = path16.basename(configFilepath).split(".").slice(0, -1).join(".");
