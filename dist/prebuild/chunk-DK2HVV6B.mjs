...
var Server_TCP_WebSocketProcess = class extends Server_TCP_WebSocketBase {
  ...
  handleTestResult(testResultData, ws) {
    try {
      ...
      fs6.writeFileSync(testsJsonPath, testsJsonContent);
  ...
  handleTestError(testErrorData, ws) {
    try {
      ...
      fs6.writeFileSync(errorJsonPath, errorJsonContent);
...
var Server_TCP_Commands = class extends Server_TCP_FileService {
  ...
  async writeFileSync(filepath, contents, testName) {
    ...
    try {
      fs8.writeFileSync(resolvedPath, contents);
  ...
  mkdirSync(filepath) {
    fs8.mkdirSync(filepath, { recursive: true });
  }
  readFile(filepath) {
...
var Server_DockerCompose = class extends Server_TCP_Commands {
  ...
  async initializeAndStart() {
    const { setupDockerCompose } = await import("./dockerComposeGenerator-BXHUY5HX.mjs");
  ...
  async startServices() {
    if (!fs9.existsSync(this.composeFile)) {
      ...
      if (fs9.existsSync(bundlesDir)) {
        console.error(`Contents of ${bundlesDir}:`);
        try {
          const files2 = fs9.readdirSync(bundlesDir);
...
var ServerTaskManager = class extends ServerTaskManagerBase {
  constructor(configs, testName, mode2) {
    ...
    this.writeBigBoard = () => {
      ...
      fs10.writeFileSync(summaryPath, summaryData);
      if (this.webSocketBroadcastMessage) {
        this.webSocketBroadcastMessage({
          type: "summaryUpdate",
          data: this.summary
