...
// src/server/docker/dockerfileGenerator.ts
import fs3 from "fs";
...
async function generateServices(config, runtimes, webSocketPort, log, error) {
  ...
  for (const runtime of runtimes) {
    ...
    for (const [testPath, testConfig] of Object.entries(tests)) {
      ...
      const dockerfileDir = path4.join(
        "testeranto",
        "bundles",
        "allTests",
        runtime,
        path4.dirname(testPath)
      );
      const dockerfilePath = path4.join(dockerfileDir, "Dockerfile");
      serviceConfig.build = {
        context: process.cwd(),
...
async function writeComposeFile(services, testsName, composeDir, error) {
  const composeFilePath = path5.join(
    composeDir,
...
async function setupDockerCompose(config, testsName, options) {
  ...
  const composeDir = path6.join(process.cwd(), "testeranto", "bundles");
  try {
