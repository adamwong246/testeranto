...
# Override the Node.js entrypoint to use sh
ENTRYPOINT ["sh", "-c"]
...
var webDockerCompose = (config) => {
  return {
    ...
    command: [
      "sh",
      "-c",
      `echo 'Starting web build in watch mode...' &&        echo 'Installing dependencies...' &&        cd /workspace &&        npm install --legacy-peer-deps --no-audit --no-fund --ignore-scripts --no-optional || echo 'npm install may have warnings' &&        echo 'Ensuring esbuild and esbuild-sass-plugin are installed for Linux platform...' &&        npm list esbuild 2>/dev/null || npm install --no-save esbuild@0.20.1 --no-audit --no-fund --ignore-scripts --no-optional || echo 'esbuild installation may have issues' &&        npm list esbuild-sass-plugin 2>/dev/null || npm install --no-save esbuild-sass-plugin --no-audit --no-fund --ignore-scripts --no-optional || echo 'esbuild-sass-plugin installation may have issues' &&        echo 'Creating output directory...' &&        mkdir -p /workspace/testeranto/bundles/allTests/web &&        mkdir -p /workspace/testeranto/metafiles/web &&        echo 'BUNDLES_DIR env:' "$BUNDLES_DIR" &&        echo 'Starting build process for web...' &&        TEST_NAME=allTests WS_PORT=3456 npx tsx dist/prebuild/server/runtimes/web/web.ts allTests.ts dev || echo 'Build process exited with code $?, but keeping container alive for health checks'`
    ]
  };
};
