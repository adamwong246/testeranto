...
async function setupDockerCompose(config, testsName) {
    console.log(`Setting up Docker Compose for ${testsName}`);
}
async function handleRuntimeBuilds(config, testsName, testerantoDocker) {
...
export async function main() {
    ...
    const composeFilePath = path.join(process.cwd(), "testeranto", "bundles", `${testsName}-docker-compose.yml`);
    ...
    const portFilePath = path.join(process.cwd(), "testeranto", "bundles", `${testsName}-docker-man-port.txt`);
    ...
    if (initialBuildServices.length > 0) {
        ...
        setTimeout(async () => {
            await (testerantoDocker === null || testerantoDocker === void 0 ? void 0 : testerantoDocker.identifyBuildServices());
            const updatedBuildServices = (testerantoDocker === null || testerantoDocker === void 0 ? void 0 : testerantoDocker.getBuildServices()) || [];
        ...
        setTimeout(async () => {
            ...
            if (healthyServices.length > 0) {
                ...
                for (const service of healthyServices) {
                    ...
                    testerantoDocker.startTestServicesForBuildService(service.name);
                }
            }
            else {
...
export function getTesterantoDocker() {
