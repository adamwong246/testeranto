...
export async function setupWebHtmlFiles(config, testsName) {
    ...
    await Promise.all(webEndpoints.map(async (sourceFilePath) => {
        ...
        const sourceFileNameMinusJs = sourceFileName
            ...
            .slice(0, -1)
            .join(".");
        const htmlFileDir = path.join("testeranto", "bundles", "web", testsName, ...sourceDir);
        ...
        return fs.promises
            ...
            .then((x) => fs.writeFileSync(path.join(process.cwd(), htmlFilePath), webHtmlFrame(jsfilePath, htmlFilePath, cssFilePath)));
    }));
}
