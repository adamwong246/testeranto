...
export async function handleRuntimeBuilds(config, testsName, dockerMan) {
    const { nodeEntryPoints, webEntryPoints, pythonEntryPoints, golangEntryPoints, } = getRunnables(config, testsName);
    ...
    ].forEach(([runtime, keys]) => {
        keys.forEach((k) => {
            fs.mkdirSync(`testeranto/reports/${testsName}/${k
                .split(".")
                .slice(0, -1)
