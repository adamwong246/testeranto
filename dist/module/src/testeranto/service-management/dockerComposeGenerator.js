...
export async function setupDockerCompose(config, testsName) {
    ...
    if (invalidServiceNames.length > 0) {
        ...
        throw new Error("Docker Compose service names must be lowercase and alphanumeric");
    }
    const dump = {
        version: "3.8",
        services,
    ...
    const composeFilePath = path.join(composeDir, `${testsName}-docker-compose.yml`);
    try {
