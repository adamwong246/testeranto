...
export const promptPather = (entryPoint, platform, projectName) => {
    ...
    return path.join("testeranto", "reports", projectName, e, platform, `prompt.txt`);
};
...
// }
    ...
    return {
        ...
        nodeEntryPoints: Object.entries(config.node.tests).reduce((pt, cv) => {
            pt[cv[0]] = path.resolve(`./testeranto/bundles/node/${projectName}/${cv[0]
                .split(".")
                .slice(0, -1)
                .concat("mjs")
                .join(".")}`);
        ...
        webEntryPoints: Object.entries(config.web.tests).reduce((pt, cv) => {
            pt[cv[0]] = path.resolve(`./testeranto/bundles/web/${projectName}/${cv[0]
                .split(".")
                .slice(0, -1)
                .concat("mjs")
                .join(".")}`);
