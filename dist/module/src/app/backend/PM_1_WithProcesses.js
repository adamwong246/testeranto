...
import { filesHash, isValidUrl, pollForFile, puppeteerConfigs, writeFileAndCreateDir, } from "../../PM/utils.js";
import { PM_0 } from "./PM_0.js";
import { makePrompt } from "./makePrompt.js";
import { getRunnables } from "./utils.js";
const changes = {};
export class PM_1_WithProcesses extends PM_0 {
    ...
    async metafileOutputs(platform) {
        ...
        Object.keys(outputs).forEach(async (builtOutput) => {
            ...
            //   return;
            // }
            const entrypoint = await this.configTests().filter(([testEntryPoint, testPlatform]) => {
                if (testEntryPoint ===
                    builtOutput
                        .replace(`testeranto/bundles/${platform}/${this.projectName}/`, ``)
                        .replace(`.mjs`, `.ts`) &&
    ...
    async start() {
        ...
        try {
            ...
            const pythonTests = this.configTests().filter((test) => test[1] === "python");
        ...
        // pureEntryPoints,
        pythonEntryPoints, golangEntryPoints, } = runnables;
    ...
    checkQueue() {
        ...
        while (this.queue.length > 0) {
            ...
            if (!test) {
                console.error(`test is undefined ${x}, ${JSON.stringify(this.configTests(), null, 2)}`);
                process.exit(-1);
                continue;
            }
