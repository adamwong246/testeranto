        ...
        if (nodeDone && webDone && importDone) {
            ...
            try {
                ...
                for (const test of config.tests) {
                    ...
                    const buildJsonDir = path.resolve(process.cwd(), "testeranto", "reports", testName, ...entryPoint.split("/").slice(0, -1), // Get directory path without filename
                    runtime);
                    ...
                    try {
                        ...
                        fs.writeFileSync(buildJsonPath, JSON.stringify(logs, null, 2), "utf-8");
