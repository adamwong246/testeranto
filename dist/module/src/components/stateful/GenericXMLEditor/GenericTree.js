...
export const GenericTree = ({ node, selectedNodeId, onSelectNode, onAddNode, onRemoveNode, onToggleVisibility, hiddenNodes, nodeTypes }) => {
    ...
    return (React.createElement("li", { style: { listStyle: 'none', marginLeft: '0', paddingLeft: '10px' } },
        React.createElement("div", { className: `d-flex justify-content-between align-items-center p-1 ${isSelected ? 'bg-primary text-white rounded' : ''}`, onClick: () => onSelectNode(node.id), style: { cursor: 'pointer' } },
        ...
        isSelected && (React.createElement("div", { className: "mt-1" },
            React.createElement(Dropdown, null,
                ...
                React.createElement(Dropdown.Menu, null, nodeTypes.map(({ label, type }) => (React.createElement(Dropdown.Item, { key: type, onClick: () => onAddNode(node.id, type) }, label)))))))));
};
