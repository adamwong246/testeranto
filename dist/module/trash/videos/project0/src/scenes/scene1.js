...
export default makeScene2D(function* (view) {
    ...
    view.add(React.createElement(React.Fragment, null,
        ...
        React.createElement(Txt, { ref: title, text: "AI has changed the game forever", fontSize: 60, fontFamily: 'JetBrains Mono', fill: '#cdd6f4', y: -300, opacity: 0 }),
        React.createElement(Layout, { ref: codeExplosion, layout: true, direction: 'column', gap: 20, wrap: 'wrap', width: 1000, height: 600 })));
    ...
    try {
        yield* all(title().opacity(1, totalDuration * 0.08), // ~1.5s for title
    ...
    for (let i = 0; i < 50; i++) {
        ...
        codeExplosion().add(React.createElement(CodeBlock, { code: `// ${file}`, fontSize: 24, fontFamily: 'JetBrains Mono', fill: color, x: x, y: y, scale: scale, opacity: 0 }));
        codeCount(codeCount() + 1);
        yield* waitFor(explosionDuration / 50); // Divide by number of iterations
    }
    // Files appear
    yield* all(...codeExplosion().children().map(child => child.opacity(1, totalDuration * 0.08)));
    // Highlight relevant files
    const relevantFiles = codeExplosion().children().slice(0, 3);
    yield* all(...relevantFiles.map(file => file.scale(2, totalDuration * 0.08).to(1, totalDuration * 0.08)));
    // Zoom effect
    yield* all(...relevantFiles.map(file => file.scale(3, totalDuration * 0.11).to(2, totalDuration * 0.11)));
    // Final message
    yield* all(title().text("AI has changed testing forever", totalDuration * 0.11), ...relevantFiles.map(file => file.fill('#a6e3a1', totalDuration * 0.11)));
