...
exports.implementation = {
    ...
    thens: {
        hasContainerFluid: () => (state) => {
            ...
            chai_1.assert.exists(container, 'Should have container-fluid div');
            return state;
        },
        hasNavBar: () => (state) => {
            const htmlElement = state.htmlElement;
            const navBar = htmlElement.querySelector('nav.navbar');
            chai_1.assert.exists(navBar, 'Should render NavBar component');
            return state;
        },
        hasNavBarTitle: () => (state) => {
            ...
            chai_1.assert.isNotEmpty(navBarTitle, 'NavBar should have title');
            return state;
        },
        hasTestTable: () => (state) => {
            const htmlElement = state.htmlElement;
            const testTable = htmlElement.querySelector('table');
            chai_1.assert.exists(testTable, 'Should render test table');
            return state;
        },
        rendersTestSuite1: () => (state) => {
            ...
            chai_1.assert.include(testNames, 'test-suite-1', 'Should render test-suite-1');
            return state;
        },
        rendersTestSuite2: () => (state) => {
            ...
            const testNames = Array.from(testTable.querySelectorAll('tbody tr td:first-child'))
                .map(el => el.textContent);
            chai_1.assert.include(testNames, 'test-suite-2', 'Should render test-suite-2');
            return state;
        },
        unhappyPath: () => (state) => {
            const htmlElement = state.htmlElement;
            const errorAlert = htmlElement.querySelector('.alert-danger');
            chai_1.assert.exists(errorAlert, 'Should show error alert');
            const errorText = errorAlert === null || errorAlert === void 0 ? void 0 : errorAlert.textContent;
            chai_1.assert.include(errorText, 'Test error message', 'Should display error message');
            return state;
        },
