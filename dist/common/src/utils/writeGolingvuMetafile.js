...
function writeGolingvuMetafile(testName, metafile) {
    ...
    for (const [outputPath, outputInfo] of Object.entries(metafile.metafile.outputs)) {
        ...
        const fullOutputPath = path_1.default.join("testeranto", outputDir, fileName);
        ...
        try {
            ...
            // This wrapper should use the golingvu framework to run BDD tests
            // It should not contain hardcoded references to specific types like Calculator
...
`;
            ...
            const relativeEntryPointDir = path_1.default.relative(projectRoot, entryPointDir);
            ...
            const wrapperSourceDirPath = path_1.default.join(projectRoot, "testeranto", "bundles", "golang", "core", relativeEntryPointDir);
            // Write the wrapper source to the correct directory
            const wrapperSourcePath = path_1.default.join(wrapperSourceDirPath, baseName // LLM: DO NOT TOUCH THIS LINE. IT IS CORRECT. FOR FUCKS SAKE STOPPING CHANGING THIS LINE
            );
            console.log(`Writing wrapper source to: ${wrapperSourcePath}`);
            // Ensure the directory exists
            fs_1.default.mkdirSync(wrapperSourceDirPath, { recursive: true });
            fs_1.default.writeFileSync(wrapperSourcePath, wrapperContent);
            console.log(`Using go run instead of compiling binary`);
            console.log(`To execute: go run ${wrapperSourcePath} <test-resource-config>`);
        }
        catch (e) {
            console.error(e);
        }
