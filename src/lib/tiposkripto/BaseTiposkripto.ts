...
> {
  ...
  constructor(
    ...
    // Store the test resource configuration if provided
    ...
    // If testResourceConfiguration is provided, run tests immediately
    if (testResourceConfiguration) {
      this.runTestsImmediately(testResourceConfiguration);
    } else {
      // Otherwise, use the old WebSocket pattern (for backward compatibility)
