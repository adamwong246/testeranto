...
abstract class BaseGiven<ISubject, IStore, ISelection, IThenShape> {
  name: string;
...
type ITestJob = //[filename: string, classname: string]
  {
    toObj(): object;
    test: IT;
...
type ITestSpecification<ITestShape extends ITTestShape> = (
  Suite: {
    ...
    const toReturn: ITestJob[] = suites.map((suite) => {
      return {
        ...
        runner: async (pKey: string) => {
          ...
          process.on('testeranto:go', async (ap: number[]) => {
            allocatedPorts = ap;

            console.log('testeranto:go', allocatedPorts)

            await suite.run(input, { ports: allocatedPorts })

            /* @ts-ignore:next-line */
            process.send({
              type: 'testeranto:done',
              data: {
                success: true,
                allocatedPorts,
              }
          ...
          process.on('testeranto:abort', async function (allocatedPorts: number[]) {
            console.log('testeranto:abort', allocatedPorts)

            await suite.aborter();

            /* @ts-ignore:next-line */
            process.send({
              type: 'testeranto:aborted',
              data: {
                success: true,
                allocatedPorts,
              }
...
export default { Testeranto, BaseFeature, TesterantoFeatures, TesterantoGraphDirected, TesterantoGraphDirectedAcyclic, TesterantoGraphUndirected }