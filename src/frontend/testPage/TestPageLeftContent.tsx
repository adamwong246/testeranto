...
export const TestPageLeftContent = ({
  ...
  const { fileSystem, listDirectory, readFile, refresh, loading, error } =
    useFileSystemSync('.');
  ...
  return (
    <div className="p-2">
      ...
      {/* Display real-time file tree when sync is enabled */}
      {syncEnabled && (
        <div className="mt-3">
          <div className="text-muted small mb-1">Live Files</div>
          {loading && <div className="text-muted small">Loading...</div>}
          {error && <div className="text-danger small">Error: {error}</div>}
          {!loading && !error && fileSystem.files[projectName] && (
            <>
              {fileSystem.files[projectName].map((item) => (
                <div
                  key={item.path}
                ...
                >
                  <i
                    className={`bi bi-${item.type === "folder" ? "folder" : "file"
                      } me-1`}
                  ></i>
                  {item.name}
                </div>
