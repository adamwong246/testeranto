...
// It shows an older implementation of PM_DockerManager
// it needs to treat tests differently based on their runtimes

/// For node, golang, and python, we use TCP and invoke them normally

/// For web, we need something more complex.
/// ATM these web tests break because we are running them like node tests. Web tests need to be run in the browser.
/// PM_DockerMan exposes functions in the page, with which the web tests can communicate, as an alternative to TCP
/// We have a choice to make then
/// 1) We bundle every web test with chromium and invoke the test by accessing the test through a page through Chromium
/// 2) We have only 1 instance of Chromium, which DockerMan will invoke, pointed at the URL of the test file.
