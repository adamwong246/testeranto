...
export class DockerMangerPM extends PM {
  mode: IMode;
  browser: Browser;
  configs: IBuiltConfig;
  projectName: string;
  testResourceConfiguration: any;
  private commandHandlers: Map<string, (...args: any[]) => any>;
  ...
  // Implement abstract methods from PM
  async start(): Promise<void> {
    // TCP server is started elsewhere
  }
  ...
  async mkdirSync(fp: string) {
    try {
      ...
      console.error(`Error creating directory ${fp}:`, error.message);
      return false;
    }
  }
  ...
  async typeInto(selector: string, value: string): Promise<any> {
    // Use the first available page
    const pages = await this.browser.pages();
    if (pages.length === 0) {
      throw new Error("No pages available");
  ...
  // Keep the original implementations with page parameter for internal use
  async clickWithPage(selector: string, page: Page) {
    return page.click(selector);
  }

  async focusOnWithPage(selector: string, p: string) {
    this.doInPage(p, (page) => {
      return page.focus(selector);
    });
  }

  async typeIntoWithPage(value: string, p: string) {
    this.doInPage(p, (page) => {
      return page.keyboard.type(value);
    });
  }
