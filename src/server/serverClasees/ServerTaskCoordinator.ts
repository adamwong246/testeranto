...
export class ServerTaskCoordinator extends Server_DockerCompose {
  ...
  updateWebProcessStatus = (
    ...
    const message = status === 'completed' 
      ...
      : `Web test failed: ${webProcess?.testName || processId} - ${error || 'Unknown error'}`;
    
    this.addLogEntry(processId, status === 'completed' ? 'stdout' : 'stderr', message);
  };
}
