...
import { Server_Browser } from "./Server_Browser";
...
export class Server_MetafileWatcher extends Server_Browser {
  ...
  private initializeMetafilePaths(): void {
    ...
    if (this.configs.src) {
      ...
      runtimes.forEach(runtime => {
        const metafilePath = path.join(baseDir, srcDir, "testeranto", "metafiles", runtime, "allTests.json");
  ...
  private startWatchingMetafiles(): void {
    ...
    const existingMetafiles = this.metafilePaths.filter(file => {
      ...
      if (!exists) {
        console.log(ansiC.yellow(`Metafile does not exist, will watch when created: ${file}`));
      }
      return exists;
    });

    if (existingMetafiles.length === 0) {
      console.log(ansiC.yellow("No existing metafiles found to watch. Will watch for creation."));
    }

    // Watch for changes in metafile directories
    const watchDirs = [
      path.join(process.cwd(), "testeranto", "metafiles"),
      ...(this.configs.src ? [path.join(process.cwd(), this.configs.src, "testeranto", "metafiles")] : [])
    ...
    watchDirs.forEach(dir => {
      ...
      watcher
        ...
        .on('change', (filePath) => this.handleMetafileChange('change', filePath))
        .on('unlink', (filePath) => this.handleMetafileChange('unlink', filePath))
        .on('error', (error) => console.error(ansiC.red(`Watcher error: ${error}`)));
    ...
    existingMetafiles.forEach(metafile => {
      ...
      watcher
        .on('change', (filePath) => this.handleMetafileChange('change', filePath))
        .on('unlink', (filePath) => this.handleMetafileChange('unlink', filePath))
        .on('error', (error) => console.error(ansiC.red(`Metafile watcher error: ${error}`)));

      this.watchers.push(watcher);
      console.log(ansiC.blue(`Watching metafile: ${metafile}`));
    });

    console.log(ansiC.green(ansiC.inverse(`Started ${this.watchers.length} metafile watchers`)));
  }

  private handleMetafileChange(event: 'add' | 'change' | 'unlink', filePath: string): void {
    ...
    const isMetafile = this.metafilePaths.some(metafile => 
  ...
  private processMetafileUpdate(filePath: string): void {
    try {
      ...
      console.log(ansiC.green(`Processing ${runtime} metafile update from ${filePath}`));
  ...
  private scheduleTestsFromMetafile(metafile: any, runtime: IRunTime, sourcePath: string): void {
  ...
  private scheduleTest(testInfo: any, runtime: IRunTime, sourcePath: string): void {
    ...
    if (typeof testInfo === 'string') {
      ...
      testName = testInfo.name || testInfo.testName || testInfo.path || 'unknown';
    ...
    if (this.addToQueue) {
      ...
      console.log(ansiC.magenta(`Queueing test: ${testName} (${runtime}) from ${src}`));
      ...
      this.addToQueue(
        ...
        this.configs,
        this.projectName,
        this.cleanupTestProcessesInternal?.bind(this) || ((testName: string) => {}),
        this.checkQueue?.bind(this) || (() => {}),
        addableFiles
      );
    } else {
      console.log(ansiC.yellow(`Cannot queue test ${testName}: addToQueue not available`));
    }
  }
