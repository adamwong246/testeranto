...
As the builder services produce bundles and set of input files, the server watches for those changes. When a bundle changes, a static analysis and BDD test should be scheduled. The static analysis is defined on the config 'allTests.ts', whereas the bdd test is supplied by testeranto
