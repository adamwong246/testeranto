...
export class TuiFramework extends EventEmitter {
  ...
  private createTab(title: string, command: string, args: string[], parentId?: number): Tab {
  ...
  private updateTabBar(): void {
    ...
    const items = visibleTabs.map(tab => {
      ...
      const maxTitleLength = (this.tabBar.width as number) - 6 - indent.length - expandIndicator.length;
      let title = tab.title;
  ...
  private createTestTab(title: string, command: string, args: string[], parentId?: number): Tab {
...
export function createTui(program: Command, options?: TuiOptions, config?: any): TuiFramework {
