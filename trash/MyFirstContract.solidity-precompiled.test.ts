...
import { SolidityPrecompiledTesteranto } from "../subPackages/solidity/solidity-precompiled.testeranto.test";
import { commonGivens } from "../src/MyFirstContract.test";
...
const implementation: ITestImplementation<
  ...
  {
    givens: {
      [K in keyof ISpec["givens"]]: (...Iw: ISpec["givens"][K]) => void;
    };
    whens: {
      [K in keyof ISpec["whens"]]: (...Iw: ISpec["whens"][K]) => ISpec["when"];
    };
    checks: {
      [K in keyof ISpec["checks"]]: (...Iw: ISpec["checks"][K]) => void;
    };
  }
> = {
  suites: {
...
export const MyFirstContractPrecompiledTesteranto =
  SolidityPrecompiledTesteranto(implementation, specification, [
    "MyFirstContract",
    async (web3) => {
