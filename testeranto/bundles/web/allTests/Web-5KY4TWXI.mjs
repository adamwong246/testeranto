...
var Tiposkripto = class {
  ...
  async connectWebSocket(port, host) {
    const wsHost = host || process_exports.env.WS_HOST || "localhost";
    const protocol = "ws";
    ...
    console.log(`[Tiposkripto] Process ID: ${process_exports.pid}`);
...
var WebTiposkripto = class extends Tiposkripto {
  ...
  async receiveTestResourceConfig(partialTestResource) {
    ...
    return await this.testJobs[0].receiveTestResourceConfig(new (void 0)(config));
  }
};
