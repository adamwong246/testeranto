...
var Tiposkripto = class {
  constructor(input, testSpecification, testImplementation, testResourceRequirement = defaultTestResourceRequirement, testAdapter = {}, uberCatcher, port) {
    this.artifacts = [];
    // puppetMaster: IPM;
    // WebSocket handling
    this.ws = null;
  ...
  async connectWebSocket(port) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        timeoutFired = true;
        console.log(`[Tiposkripto] \u274C WebSocket connection timeout after 10 seconds to ${url}`);
        console.log(`[Tiposkripto] This usually means:`);
        console.log(`[Tiposkripto]   1. No WebSocket server is running on port ${port}`);
        console.log(`[Tiposkripto]   2. The server is not accepting connections`);
        console.log(`[Tiposkripto]   3. There's a firewall blocking the connection`);
        console.log(`[Tiposkripto] Please ensure a WebSocket server is running on port ${port}`);
      ...
      this.ws.on("open", () => {
        ...
        console.log(`[Tiposkripto] \u2705 WebSocket connected successfully to ${url}`);
      ...
      this.ws.on("error", (error) => {
        ...
        console.log(`[Tiposkripto] This usually means the WebSocket server is not running`);
        console.log(`[Tiposkripto] Check if something is listening on port ${port}`);
        reject(error);
      });
      this.ws.on("message", (data) => {
        const dataStr = data.toString();
        console.log(`[Tiposkripto] \u{1F4E8} Received WebSocket message (${dataStr.length} chars)`);
        if (dataStr.length > 500) {
          console.log(`[Tiposkripto] Message preview: ${dataStr.substring(0, 500)}...`);
        } else {
          console.log(`[Tiposkripto] Message: ${dataStr}`);
        }
        try {
          const message = JSON.parse(dataStr);
          console.log(`[Tiposkripto] Parsed message type: ${message.type || "unknown"}`);
          ...
          console.error(`[Tiposkripto] Error parsing WebSocket message:`, error);
        }
      });
...
var tiposkripto = async (input, testSpecification, testImplementation, testAdapter, testResourceRequirement = defaultTestResourceRequirement) => {
  try {
    console.log(`[Node] Creating Tiposkripto instance with WebSocket port: 3456`);
